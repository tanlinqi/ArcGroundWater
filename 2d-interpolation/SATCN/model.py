...
class STower(nn.Module):
    ...
    def __init__(self, in_features, out_features, aggregators, scalers,  avg_d,
        ...
        self.Theta_po = nn.Parameter(torch.FloatTensor(len(aggregators) * len(scalers) * self.in_features, self.out_features))
        self.bias_po = nn.Parameter(torch.FloatTensor(self.out_features))
...
class SAGE(nn.Module):
    ...
    def __init__(self, in_features, out_features, aggregators = ['mean'],
        ...
        self.out_features = out_features
        self.aggregators = aggregators
        self.Theta_po = nn.Parameter(torch.FloatTensor(self.in_features, self.out_features))
        self.bias_po = nn.Parameter(torch.FloatTensor(self.out_features))
        self.reset_parameters()
